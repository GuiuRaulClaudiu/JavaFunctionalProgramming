...
public class Pack_4_Streams_Difficult {
    ...
    public void exercise_1_findFirst() {
        ...
        result = EMPLOYEES.stream().filter(e -> EMPLOYEES.stream()
                        .anyMatch(x -> e.getFirstName().equals(x.getFirstName())
                                && e.getSurname().equals(x.getSurname())
                                && !e.equals(x)))
                .map(x -> x.getFirstName() + " " + x.getSurname()).findFirst().get();
    ...
    public void exercise_2_groupingBy_counting() {
        ...
        result = EMPLOYEES.stream()
                .collect(groupingBy(x -> x.getHomeAddress().getPostCode().substring(0, 2)))
                .values().stream().filter(x -> x.size() >= 5).count();
    ...
    public void exercise_4_groupingBy_summingInt() {
        ...
        result = EMPLOYEES.stream()
                .collect(groupingBy(x -> x.getCompany().getName(), Collectors.summingInt(y -> y.getSalary().intValue())))
    ...
    public void exercise_6_mapToLong() {
        ...
        result = IntStream.range(0, rows.length)
                .mapToLong(i -> IntStream.range(0, columns.length).mapToLong(j -> rows[i]*columns[j]).sum()).sum();
